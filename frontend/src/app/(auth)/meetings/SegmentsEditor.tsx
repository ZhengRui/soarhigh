...
export function SegmentsEditor({
  ...
  // Compute the positive gap (buffer) between the previous segment's end
  // and this segment's start. Zero or negative gaps (overlaps are handled
  // via hasTimeConflict) render no pill.
  const bufferBefore = (index: number): number => {
    if (index <= 0) return 0;
    const prev = segments[index - 1];
    const cur = segments[index];
    const prevEnd =
      timeStringToMinutes(prev.start_time) + parseInt(prev.duration);
    const curStart = timeStringToMinutes(cur.start_time);
    return Math.max(curStart - prevEnd, 0);
  };
    ...
    <div>
      <div className='space-y-4'>
        {segments.map((segment, index) => {
          ...
          const buf = bufferBefore(index);

          return (
            <div key={segment.id}>
              {buf > 0 && (
                // Left-aligned with the time text: outer padding matches the
                // segment card's px-2 sm:px-4, so the pill's left edge lines
                // up with the "20:57" label inside the card above/below.
                // -mt-3 mb-1 compensates for the asymmetric vertical space:
                // space-y-4 puts 16px ABOVE this wrapper but nothing directly
                // below (the following card's py-4 handles that), so without
                // asymmetric pull the pill sits visibly below the midpoint.
                <div className='px-2 sm:px-4 -mt-4 mb-2 select-none'>
                  <span className='inline-block text-[10px] font-medium text-amber-700 px-2 py-0.5 rounded-full bg-amber-50 border border-amber-200 leading-none'>
                    {buf} min
                  </span>
                </div>
              )}
              <div className='group relative flex flex-col sm:flex-row gap-4 items-start py-4 px-2 sm:px-4 rounded-lg'>
                <div
                        ...
                        <div className='py-1 max-h-60 overflow-auto'>
                          {Object.keys(SEGMENT_TYPE_MAP).map((type) => (
                            <div
                              ...
                              onClick={() =>
                                handleSegmentTypeChange(index, type)
                              }
                            >
                              {type}
                            </div>
