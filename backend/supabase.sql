...
-- =============================================
-- Scratch tables for AI agents. These are NOT the canonical meeting data; the
-- user still clicks Save to write into meetings / attendees / etc. These
-- tables hold the conversation, per-turn agenda snapshots, and the Pydantic AI
-- ModelMessage cursor so a session can be resumed, reverted, or served from
-- multiple channels (web, Telegram, Hermes).
--
-- One row per user message. agent_kind / route distinguishes router-only,
-- meeting, and statistics turns within a single conversation.
...
-- Public Agent tables. Separate from member Agent history: member Agent
-- sessions are owned by auth.users.id, while Public Agent sessions are owned
-- by (channel, visitor_key), e.g. (miniapp, wxid) or future
-- (web, server-issued visitor id). Public endpoints use the backend
-- service-role client and enforce ownership in application code.

CREATE TABLE agent_sessions_public (
    session_id   TEXT PRIMARY KEY,
...
CREATE TABLE agent_turns_public (
...
CREATE INDEX idx_agent_sessions_public_visitor_created
    ON agent_sessions_public(channel, visitor_key, created_at);

CREATE INDEX idx_agent_turns_public_session_created
...
CREATE TABLE agent_rate_limits_public (
    key          TEXT NOT NULL,
